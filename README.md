...
Ingresse na pasta buscador do repositorio clonado e copie o arquivo 'buscador.php' no diretorio raiz onde seu wordpress foi instaldo.
...
### 3. Copie arquivo connection.php e faça as alterações necessárias

Ingresse na pasta buscador do repositorio clonado e copie o arquivo 'connection.php' na pasta 'wp-content/themes/astrid/' do diretorio raiz onde seu wordpress foi instaldo.

Após fazê-lo, altere o arquivo para inserir as credencias do seu banco de dados:
...
### 4. Altere o arquivo search.php

Ingresse na pasta buscador do repositorio clonado e copie o conteúdo do arquivo 'search.php'. No diretório raiz do seu site Wordpress, vá para `wp-content/themes/astrid/search.php`, apague e cole o conteúdo copiado.

### 5. Crie o arquivo custom-scripts.js

Ingresse na pasta buscador do repositorio clonado e copie o conteúdo do arquivo 'custom-scripts.js'. No diretório raiz do seu site Wordpress, vá para `wp-content/themes/astrid/js`, crie um arquivo chamado 'custom-scripts.js' e cole o conteúdo copiado.

### 6. Altere o arquivo functions.php

Ingresse na pasta buscador do repositorio clonado e copie o conteúdo do arquivo 'functions.php'. No diretório raiz do seu site Wordpress, vá para `wp-content/themes/astrid/functions.php` e, ao final do arquivo, cole o conteúdo copiado.

### 7. Estruture o banco de dados
...
### 8. Ingresse os dados no banco de dados
...
O banco de dados já esta com os dados prontos para ser usados. Ir diretamente para o passo (9. Insira o CSS customizado\_).

### 8.1 Atualize o banco de dados
...
### 8.2 alternativa XML para preencher o Banco de dados
...
Para implementação desse arquivo foram realizados os seguintes passos:
No diretório raiz do Wordpress, o arquivo buscador.php foi criado, contendo um formulário de pesquisa HTML e código PHP para carregar o template e as configurações do Wordpress que estão sendo usados. Tal formulário contém o atributo action, que redireciona o usuário para a página onde aparecem os resultados, filtros de pesquisa que foram considerados interessantes para o tipo de busca executada, um campo de texto e dois campos do tipo 'hidden' (escondido), que não são visíveis para o usuário. Um deles tem o valor 'buscador' e serve como identificador e o outro é um campo padrão do Wordpress para formulários de pesquisa. Também, no diretório raiz do Wordpress em wp-content/themes/NOME_DO_SEU_TEMA, foi criado o arquivo connection.php, contendo o código neste repositório em buscador/connection.php. Sua função é estabelecer uma conexão com o banco de dados que está sendo usado por meio de uma instância da classe wpdb (conector a banco de dados do Wordpress). Em todos os arquivos em que uma consulta ao banco de dados é feita, esse arquivo é incluído. Além disso, ainda no diretório raiz do Wordpress, em wp-content/themes/NOME_DO_SEU_TEMA, o arquivo search.php, cuja função é mostrar os resultados de pesquisa, foi alterado das seguintes formas: primeiramente, uma condição foi criada para averiguar se o formulário que foi enviado se trata do buscador ou de um formulário qualquer do Wordpress. Caso seja o buscador, uma busca no banco de dados é executada usando o método get_results() do wpdb de acordo com os parâmetros enviados no formulário e os resultados são retornados como HTML. Contudo, para um formulário padrão do Wordpress, o comportamento usual é mantido. Também, no diretório raiz do Wordpress, em wp-content/themes/NOME_DO_SEU_TEMA/js, um arquivo 'custom-scripts.js', que está neste repositório em buscador/custom-scripts.js, foi criado com o javascript necessário para funcionamento do formulário e em wp-content/themes/NOME_DO_SEU_TEMA/functions.php, foi adicionado o código que está neste repositório em buscador/functions.php e serve para inserir no website o código javascript mencionado acima.
